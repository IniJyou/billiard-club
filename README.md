...
| 数据库 | MySQL 8 |
| 运行与构建 | JDK 17 + Maven（后端）；Node.js + npm + Vite 5（前端） |
...
请先准备 JDK 17、Maven、Node.js（含 npm）和 MySQL 8，并确认 MySQL 服务已启动。
...
普通用户：注册/登录 → 在线办理会员 → 查看卡号、余额与折扣 → 模拟微信/支付宝充值 → 查看球桌状态及已约时段 → 预约或取消当天球桌 → 到店由工作人员开台、结账或注销会员 → 查看个人流水。
...
| 成员 | 负责 | 产出 |
|---|---|---|
| 组长 A | 架构搭建 + 数据库整体设计 + 会员/充值模块 + 需求分析主导 | 会员建档、在线办理、充值与注销 + ER/表结构 + 用例图 |
| 成员 B | 球桌 + 预约 + 开台结账核心计费 | 球桌状态/预约时段/会话/账单 + 预约冲突与计费算法 + 序列图 |
| 成员 C | 登录权限 + 用户资料 + 流水查询 + 报表 + 前端公共组件 | 鉴权/个人资料 + 流水/报表 + 页面原型 + 测试 |

## 约定

- 统一返回体 `Result<T>`、统一异常处理（见 `common/` 包）。
- 业务接口需要登录；访问不符合当前角色权限的接口返回 HTTP 403，登录失效返回 HTTP 401。
