...
- **安全**：密码 MD5 加密存储（可升级为 BCrypt）；基于角色的功能权限控制；关键操作记录操作员，日志禁止记录密码和完整请求体。
- **可审计性**：记录 API 状态码、操作员、客户端 IP、耗时和关键业务事件；请求以 `X-Request-Id` 关联访问日志与业务日志。
...
日志使用 SLF4J + Logback：`RequestLogInterceptor` 记录所有 `/api/**` 访问，Controller 记录登录、会员、充值、球桌和结账等成功业务事件，鉴权与登录服务记录认证失败事件。日志分别写入 `billiard-club.log`、`access.log` 和 `error.log`，每 10MB 或跨日滚动并保留 30 天；MyBatis SQL 通过 SLF4J 统一输出，便于按请求编号排查问题。
...
| 统计报表 | 收入/充值/客群/支付/利用率 | ReportController | ReportService | 各 Mapper 聚合 | Report.vue |
...
- 登录页、响应式主框架（桌面侧边栏/移动抽屉）、会员列表/编辑、球桌看板、开台结账页、充值页、流水查询页和管理员经营报表页。公共页面标题、查询面板、指标卡和图表容器保持统一交互与样式。
...
| 流水 | GET `/api/records/recharges`、`/consumptions` | 按关键词、日期、支付方式、操作员分页检索 |
| 流水 | GET `/api/records/operators` | 获取包含停用账号的历史操作员筛选项 |
| 流水 | GET `/api/records/recharges/export`、`/consumptions/export` | 按筛选条件导出 UTF-8 CSV |
| 报表 | GET `/api/reports/overview` | 管理员按日期范围查看经营报表 |
...
- 初始化数据沿用 MD5 以兼容课程旧库，账号首次成功登录后自动改存 BCrypt；未知哈希格式拒绝登录。数据库密码仍应在部署时通过安全配置管理。
- 流水查询与导出对管理员、前台开放；经营报表和球桌维护接口由后端 `@AdminOnly` 强制限制为管理员，前端同时进行路由与菜单控制。
- 报表消费收入取结账单实收金额，充值实收不含赠送；球桌利用率按非取消开台与所选日期区间重叠的实际分钟计算。数据库没有维护时段历史，因此维护时间不从可用分钟分母中扣除。
...
| 成员 C（提交前替换真实姓名） | 登录权限、流水、经营报表、前端公共布局与测试 | 负责鉴权、报表和公共组件 |
...
- 构建：`mvn test`、`mvn package`、`npm run test`、`npm run build` 均成功后才能生成提交包。
